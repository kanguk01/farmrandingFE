...
const LogoImageKeywords: React.FC = () => {
  ...
  const cropKeywords = state?.cropKeywords || [];
  ...
  const handleNext = () => {
    console.log('선택된 로고 이미지 키워드:', selectedKeywords);
    navigate('/brand-name-generation', {
      state: {
        brandingKeywords: brandingKeywords,
        cropKeywords: cropKeywords,
        logoKeywords: selectedKeywords
      }
    });
  };
    ...
    <PageContainer>
      ...
      <ContentArea>
        <MainContent>
          ...
          <KeywordContainer>
            <KeywordGrid>
              {LOGO_IMAGE_KEYWORDS.map((keyword) => (
                <KeywordTag
                  key={keyword.id}
...
export default LogoImageKeywords; 